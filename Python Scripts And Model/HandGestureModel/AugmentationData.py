...
#mv scaled*.png HandGestureData/Augmented/None
#mv translated*.png HandGestureData/Augmented/None


def saveImagesResult(img,name):

    for i in range(len(img)):
        code = i
        finalName = name + str(i)
        mpimg.imsave(finalName, img[i])




def createFilePathsTab(directoryName):

    file_paths = []
    fileNames = os.listdir("HandGestureData/" + directoryName)
    for i in range(len(fileNames)): 
        if (fileNames[i] != "raw_data.txt"):
            file_paths.append('HandGestureData/' + directoryName + "/" + fileNames[i])
    return file_paths

IMAGE_SIZE = 50

file_paths = createFilePathsTab("GoLeftRot")
...
def tf_resize_images(X_img_file_paths):
    ...
    return X_data
X_imgs = tf_resize_images(file_paths)
...
saveImagesResult(scaled_imgs,"scaled")
...
saveImagesResult(translated_imgs,"translated")
