...
public class HandGesture extends JFrame implements Runnable, MouseListener, KeyListener{
	...
	/////////////////////////////////////
	private String realTimePath = "../../realTime.png"; //PICK A REAL TIME PATH LOCATION. Python's client will need this location like "C:/Users/Pabla/Desktop/ImageAnalysis/Tests/AdvancedCarModel/real_time.png"
	/////////////////////////////////////
	...
	private String[] gestureTypes = new String[]{"Land","None","TakeOff","GoLeft","GoUp", "GoDown", "GoBack", "ComeForward", "None"}; //types of gestures
	...
	private int gestureIndex = 7; //Gesture index to collect data for
    ...
    /**
     * Initializes webcam, buffered image, 2Dsocket pixel raster, server and robot.
    ...
    public void paint(Graphics graphic) {
    	...
    	BufferedImage newImage = new BufferedImage(96, 96, BufferedImage.TYPE_INT_ARGB); //50px by 50px image that will be fed into the neural network
    	...
    	if(listOfFoundObjects.size()>0) {
        	...
        	g.drawImage(crop, (int)0, (int)0, (int)96, (int)96, (int)0, (int)0, crop.getWidth(),
        			crop.getHeight(), null);
    	...
    	if(connected == true) {
    		...
    		if(detectNoneWithNeuralNetwork == false && listOfFoundObjects.size()==0) {
    			...
    			try {
					...
					//System.out.print(newImage.getHeight() + "  " + newImage.getWidth());
    ...
    public boolean strictSkinPixelRule(float[] hsb) {
    	if(hsb[0] >0.10f && hsb[0]<0.535f && hsb[1]>0.3f && hsb[2]<1f) { //skin values : 0.15f 0.2f 0.63f 	green values : 0.15 0.5 0.3 0.9
    		return true;
    	}
    	else {
    		return false;
    ...
    public boolean looseSkinPixelRule(float[] hsb) {
    	if(hsb[0]>0.15f && hsb[0]<0.535f && hsb[1]>0f && hsb[2]<1f) { //skin values : 0.4f 1f 0.7f
    		return true;
    	}
    	else {
    		return false;
